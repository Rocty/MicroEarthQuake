...
class KTPicker():
  ...
  """
    ...
    def __init__(self, t_win=1, t_ma=10, nsigma=6, t_up=0.2, nr_len=2, nr_coeff=2, pol_len=10, pol_coeff=10,
                 uncert_coeff=3):
        """
  ...
  """
    ...
    """
        ...
        for i in range(len(trigger_ptnl)):
            ...
            M = min(r, R)
            if N * np.std(self.tr.data[
                          int(round(trigger_ptnl[i] / dt, 0)) - M:int(round(trigger_ptnl[i] / dt, 0))]) >= np.std(
                    self.tr[int(round(trigger_ptnl[i] / dt, 0)):int(round(trigger_ptnl[i] / dt, 0)) + M]):
    ...
    """
        ...
        for i in range(len(picks)):
            ...
            if self.tr[index + 1] - self.tr[index0] > 0 and abs(
                    self.tr[index + 1] - self.tr[index0]) > self.picker.pol_coeff * np.std(
    ...
    """
        ...
        for i in range(len(picks)):
            plt.plot([(picks[i] - self.tr.stats.starttime), (picks[i] - self.tr.stats.starttime)],
                     [min(self.tr), max(self.tr)], 'k--')
            plt.text((picks[i] - self.tr.stats.starttime), max(self.tr) - 0.3 * (max(self.tr) - min(self.tr)),
