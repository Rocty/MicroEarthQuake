...
class Ratio():
    ...
    """
        ...
        if self.statistics_mode == 'rms':
            ...
            rmsE = np.clip(rmsE, 1.e-19,
                           1.e19)  # clip the sigmaE to avoid zeros values since FC encounters invalid value if denominator sigmaE is zero
        if self.statistics_mode == 'std':
            sigmaE[:, npts_t_long:LEN] = np.std(tmp, axis=-1)
            sigmaE = np.clip(sigmaE, 1.e-19,
                             1.e19)  # clip the sigmaE to avoid zeros values since FC encounters invalid value if denominator sigmaE is zero
