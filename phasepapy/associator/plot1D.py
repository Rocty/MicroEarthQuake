...
class Plot():
    ...
    def cluster_plot(self, assoc_ot_uncert=3):
        ...
        for i in range(L_ots):
            cluster = self.assoc_db.query(Candidate).filter(Candidate.ot >= candidate_ots[i].ot).filter(
                Candidate.ot < (candidate_ots[i].ot + dt_ot)).order_by(Candidate.ot).all()
            cluster_sta = self.assoc_db.query(Candidate.sta).filter(Candidate.ot >= candidate_ots[i].ot).filter(
                Candidate.ot < (candidate_ots[i].ot + dt_ot)).order_by(Candidate.ot).all()
        ...
        plt.ylim(0, max(y1) + 1);
        plt.xlim(min(x2) - timedelta(seconds=60), max(x2) + timedelta(seconds=60))
    ...
    """
        ...
        # =============================================================
        ### Basemap Module
        m = Basemap(projection='merc', llcrnrlat=south, urcrnrlat=north, llcrnrlon=west, urcrnrlon=east, lat_ts=0,
                    resolution='c')
        m.drawcountries()
        m.drawstates()
        ...
        matches = self.assoc_db.query(Candidate).filter(Candidate.assoc_id == assoc_id).filter(
            Candidate.locate_flag == True).all()
        mismatches = self.assoc_db.query(Candidate).filter(Candidate.assoc_id == assoc_id).filter(
            Candidate.locate_flag == False).all()
        lon_eve, lat_eve = self.assoc_db.query(Associated.longitude, Associated.latitude).filter(
        ...
        single_phases = self.assoc_db.query(PickModified).filter(PickModified.assoc_id == assoc_id).filter(
    ...
    """
        if phase == 'P':
            min = self.tt_stations_db_1D.query(TTtable1D).filter(TTtable1D.p_tt <= time).order_by(
                TTtable1D.p_tt.desc()).first()
            max = self.tt_stations_db_1D.query(TTtable1D).filter(TTtable1D.p_tt >= time).order_by(
                TTtable1D.p_tt).first()
        ...
        if phase == 'S':
            min = self.tt_stations_db_1D.query(TTtable1D).filter(TTtable1D.s_tt <= time).order_by(
                TTtable1D.s_tt.desc()).first()
            max = self.tt_stations_db_1D.query(TTtable1D).filter(TTtable1D.s_tt >= time).order_by(
                TTtable1D.s_tt).first()
    ...
    def section_plot(self, assoc_id, files, seconds_ahead=5, record_length=100, channel='Z'):
        ...
        station_single = self.assoc_db.query(Pick.sta).filter(Pick.assoc_id == assoc_id).filter(
            Pick.locate_flag == None).all()
        ...
        for tr in ST:
            if tr.stats.channel[2] in Chan and tr.stats.station in sta_list:
                if tr.stats.starttime.datetime < eve.ot and tr.stats.endtime.datetime > eve.ot:
                    tr.trim(UTCDateTime(eve.ot - timedelta(seconds=seconds_ahead)),
        ...
        for tr in ST_new:
            ...
            t = np.arange(0, round(
                tr.stats.npts / tr.stats.sampling_rate / tr.stats.delta)) * tr.stats.delta  # due to the float point arithmetic issue, can not use "t=np.arange(0,tr.stats.npts/tr.stats.sampling_rate,tr.stats.delta)"
            segs.append(np.hstack((data[:, np.newaxis], t[:, np.newaxis])))
            lon, lat = self.tt_stations_db_1D.query(Station1D.longitude, Station1D.latitude).filter(
                Station1D.sta == tr.stats.station).first()
            distance = int(gps2dist_azimuth(lat, lon, eq_lat, eq_lon)[
                               0] / 1000.)  # gps2DistAzimuth return in meters, convert to km by /1000
            ...
            if channel == 'Z3':
                picks_p = self.assoc_db.query(Pick.time).filter(Pick.assoc_id == assoc_id).filter(
                    Pick.sta == tr.stats.station).filter(Pick.chan == tr.stats.channel).filter(Pick.phase == 'P').all()
                if not picks_p:
                    picks_p = self.assoc_db.query(PickModified.time).filter(PickModified.assoc_id == assoc_id).filter(
                        PickModified.sta == tr.stats.station).filter(PickModified.phase == 'P').all()
                picks_s = self.assoc_db.query(PickModified.time).filter(PickModified.assoc_id == assoc_id).filter(
                    PickModified.sta == tr.stats.station).filter(PickModified.phase == 'S').all()
            ...
            for pick, in picks:
                index = int((pick - eve.ot + timedelta(
                    seconds=seconds_ahead)).total_seconds() / tr.stats.delta)  # ;print pick,eve.ot,index,len(data)
