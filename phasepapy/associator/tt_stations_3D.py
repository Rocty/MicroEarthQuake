...
class TTtable3D(BaseTT3D):
    ...
    def __repr__(self):
        return "<TTtable3D (d_km%.2f delta%.2f p%.2f s%.2f s-p%.2f sn%.2f pn%.2f sn-pn%.2f)>" % (
        self.d_km, self.delta, self.p, self.s, self.s_p, self.pn, self.sn, self.sn_pn)
...
class Station3D(BaseTT3D):
    ...
    def __repr__(self):
        ...
        self.sta, self.net, self.loc, self.latitude, self.longitude, self.starttime, self.endtime)
