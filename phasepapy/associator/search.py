            ...
            for j in range(len(nt_cpt_set)):
                for k in range(len(np_cpt_set)):
                    vars()['cpt_%d' % (i * len(nt_cpt_set) * len(np_cpt_set) + j * len(np_cpt_set) + k)] \
                        = (nr_cpt_set[i][1] - 1) * (np - 1) * (nt - 1) + (nt_cpt_set[j][1] - 1) * (np - 1) + (
                                np_cpt_set[k][1] - 1)

                    cpt_set.append(
                        (i, j, k, vars()['cpt_%d' % (i * len(nt_cpt_set) * len(np_cpt_set) + j * len(nt_cpt_set) + k)]))
        ...
        for i in range(len(Z_fine)):
            for j in range(len(Y_fine)):
                for k in range(len(X_fine)):
                    ...
                    cpt_set_f.append(
                        (i, j, k, vars()['cpt_%d' % (i * len(Y_fine) * len(X_fine) + j * len(Y_fine) + k)]))
                    # print cpt_set_f
