...
class LocalAssociator():
  ...
  """
    ...
    def __init__(self, db_assoc, db_tt, max_km=350, aggregation=1, aggr_norm='L2', assoc_ot_uncert=3, nsta_declare=3,
    ...
    """
        ...
        tmp, d_diff = tt_km(self.tt_stations_db_1D,
                            self.max_km)  # From max distance set our maximum travel_time
    ...
    """
        ...
        for sta, in stations:  # the comma is needed
            picks = self.assoc_db.query(Pick).filter(Pick.sta == sta).filter(Pick.assoc_id == None).order_by(
                Pick.time).all()
            ...
            picks_modified = self.assoc_db.query(PickModified).filter(PickModified.sta == sta).filter(
            ...
            for i in range(0, len(picks_modified) - 1):
                for j in range(i + 1, len(picks_modified)):
                    ...
                    if s_p <= self.max_s_p and s_p >= self.min_s_p:
                        ...
                        new_candidate = Candidate(ot, sta, tt.d_km, tt.delta, picks_modified[i].time,
                                                  picks_modified[i].id, picks_modified[j].time, picks_modified[j].id)
    ...
    """
        ...
        for i in range(L_ots):
            cluster = self.assoc_db.query(Candidate).filter(Candidate.assoc_id == None).filter(
                Candidate.ot >= candidate_ots[i].ot).filter(Candidate.ot < (candidate_ots[i].ot + dt_ot)).order_by(
                Candidate.ot).all()
            cluster_sta = self.assoc_db.query(Candidate.sta).filter(Candidate.assoc_id == None).filter(
                Candidate.ot >= candidate_ots[i].ot).filter(Candidate.ot < (candidate_ots[i].ot + dt_ot)).order_by(
                Candidate.ot).all()
        ...
        for i in range(len(Array)):
            ...
            if Array[i][1] >= self.nsta_declare:
                candis = self.assoc_db.query(Candidate).filter(Candidate.assoc_id == None).filter(
                    Candidate.ot >= candidate_ots[index].ot).filter(
                    Candidate.ot < (candidate_ots[index].ot + dt_ot)).order_by(Candidate.ot).all()

                # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                # remove the candidates with the modified picks has been associated
                picks_associated_id = list(
                    set(self.assoc_db.query(PickModified.id).filter(PickModified.assoc_id != None).all()))
                index_candis = []
                ...
                for i, candi in enumerate(candis):
                    ...
                    lon, lat = self.tt_stations_db_1D.query(Station1D.longitude, Station1D.latitude).filter(
                        Station1D.sta == candi.sta).first()
                ...
                for i in range(len(cb)):
                    ...
                    if len(radius_cb) >= self.nsta_declare:  # self.nsta_declare has to be greater than or equal to 3
                        location = fmin(locating, [lon, lat], radius_cb,
                                        disp=0)  # disp = 1 disp : bool, Set to True to print convergence messages.
                ...
                if rms_sort:
                    ...
                    lat = loc[1]
                    matches = cb[
                        index]  # matches is one of combination of radius.append([candi.sta, lon, lat, candi.d_km, candi.delta, i])
                    # print 'location: ', lat, lon, rms
                    # print 'matches',matches

                    # cut off outlier
                    MISMATCHES = []
                    MATCHES_nol, mismatches = outlier_cutoff(matches, loc,
                                                             self.cutoff_outlier)  # MATCHES_nol is the matches of no outlier, MISMATCHES is the outliers,
                    ...
                    if nsta >= self.nsta_declare:
                        ...
                        if RMS <= self.loc_uncert_thresh:
                            new_event = Associated(origintime, round(ot_unc, 3), LAT, LON, round(RMS, 3), nsta,
                                                   t_create, t_update)
                            ...
                            logging.info(str(
                                ['ot:', origintime, 'ot_uncert:', ot_unc, 'loc:', LAT, LON, 'loc_uncert:', RMS, 'nsta:',
                                 nsta]))
    ...
    def single_phase(self):
        ...
        for event in events:
            ...
            for sta, in self.assoc_db.query(PickModified.sta).filter(
                    PickModified.assoc_id == event_id).distinct().all():  # only associated single phase from stations not contribute p and s pairs
                sta_assoc.append(sta)

                # associate single phase
            for sta, in self.assoc_db.query(PickModified.sta).filter(PickModified.assoc_id == None).filter(
                    PickModified.time > ot).filter(
                    PickModified.time <= (ot + timedelta(seconds=self.max_tt))).distinct().all():
                ...
                if (d_km < self.max_km) and (
                        sta not in sta_assoc):  # only associated single phase from stations not contribute p and s pairs
                    tt, d_diff = tt_km(self.tt_stations_db_1D, d_km)

                    picks_p = self.assoc_db.query(PickModified).filter(PickModified.sta == sta).filter(
                        PickModified.time >= (ot + timedelta(seconds=tt.p_tt - 0.5 * self.aggr_window))).filter(
                        PickModified.time <= (ot + timedelta(seconds=tt.p_tt + 0.5 * self.aggr_window))).all()
                    ...
                    picks_s = self.assoc_db.query(PickModified).filter(PickModified.sta == sta).filter(
                        PickModified.time >= (ot + timedelta(seconds=tt.s_tt - 0.5 * self.aggr_window))).filter(
                        PickModified.time <= (ot + timedelta(seconds=tt.s_tt + 0.5 * self.aggr_window))).all()
                    # if there are more than one modified pick in the aggr_window range, only associate the first modified pick
                    if picks_s:
    ...
    def comb(self, tt):
        ...
        cb = list(combinations((tt),
                               L))  # combinations of the array, some of them are repeated such as (sta1, sta1, sta2,...)
...
def pick_cluster(session, picks, pickwindow, pickaveraging_norm, counter):
  ...
  """
    ...
    if len(picks) == 1:
        ...
        pick_modified = PickModified(picks[0].sta, picks[0].chan, picks[0].net, picks[0].loc, picks[0].time,
                                     picks[0].phase, round(pickstd, 3), picks[0].assoc_id)
        ...
        while True:
            ...
            while True:
                ...
                if picks[i + 1].chan not in channel and (
                        picks[i + 1].time - picks[i].time).total_seconds() < pickwindow:
                ...
                elif picks[i + 1].sta == picks[i].sta and picks[i + 1].chan == picks[i].chan and picks[i + 1].time == \
                        picks[
                            i].time:  # and picks[i+1].snr==picks[i].snr and picks[i+1].phase==picks[i].phase and picks[i+1].uncert==picks[i].uncert:
                    cluster.append(picks[i + 1])
                    cluster_time.append(picks[i + 1].time)
                    channel.append(picks[i + 1].chan)
                    picks[i + 1].modified_id = counter  # assign modified id to picks
            ...
            pick_modified = PickModified(pick.sta, pick.chan, pick.net, pick.loc, pick.time, pick.phase,
            ...
            if j >= len(picks) - 1:
                if (picks[-1].time - picks[-2].time).total_seconds() > pickwindow:
                    ...
                    pick_modified = PickModified(picks[-1].sta, picks[-1].chan, picks[-1].net, picks[-1].loc,
                                                 picks[-1].time, picks[-1].phase, round(pickstd, 3), picks[-1].assoc_id)
                    session.add(pick_modified)
                    session.commit()
                else:
                    if picks[-1] in cluster:
                        ...
                        pick_modified = PickModified(picks[-1].sta, picks[-1].chan, picks[-1].net, picks[-1].loc,
                                                     picks[-1].time, picks[-1].phase, round(pickstd, 3),
                                                     picks[-1].assoc_id)
                        session.add(pick_modified)
                        session.commit()
                break
...
def locating(guess, *args):
    ...
    while True:
        ...
        residuals = residuals + (
                    gps2dist_azimuth(guess[1], guess[0], args[i][2], args[i][1])[0] / 1000 * 180 / (np.pi * 6371) -
                    args[i][4]) ** 2
...
def residuals_minimum(location, args):
    ...
    i = 0
    while True:
        residuals = residuals + (gps2dist_azimuth(location[1], location[0], args[i][2], args[i][1])[0] / 1000 * 180 / (
