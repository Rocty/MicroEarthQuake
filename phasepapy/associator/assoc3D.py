            ...
            for i in range(0, len(picks_modified) - 1):
                for j in range(i + 1, len(picks_modified)):
                    ...
                    if s_p <= self.max_s_p and s_p >= self.min_s_p:
                        ...
                        new_candidate = Candidate(ot, sta, d_km, delta, picks_modified[i].time, picks_modified[i].id,
                                                  picks_modified[j].time, picks_modified[j].id)
    ...
    """
        ...
        for i in range(len(Array)):
            ...
            if Array[i][1] >= self.nsta_declare:
                matches = self.assoc_db.query(Candidate).filter(Candidate.assoc_id == None).filter(
                ...
                for i in range(len(cb)):
                    ...
                    if len(tt_cb) >= self.nsta_declare:  # self.nsta_declare has to be greater than or equal to 3
                        tt_new, sourcegrid, rms = PyramidSearching(self.tt_stations_db_3D, self.nt, self.np, self.nr,
                                                                   tt_cb)
                ...
                if rms_sort:
                    ...
                    lat, lon, dep = self.tt_stations_db_3D.query(SourceGrids.latitude, SourceGrids.longitude,
                                                                 SourceGrids.depth).filter(
                        SourceGrids.id == sourcegrid).first()
                    ...
                    new_event = Associated(origintime, round(ot_unc, 3), lat, lon, dep, round(rms, 3), nsta, t_create,
                                           t_update)
                    ...
                    logging.info(str(
                        ['ot:', origintime, 'ot_uncert:', round(ot_unc, 3), 'loc:', lat, lon, 'rms:', round(rms, 3),
                         'nsta:', nsta]))
