...
class Pick(Base):
    ...
    def __repr__(self):
        return "Pick <%s.%s.%s.%s %s %s %s %s>" % (
        self.sta, self.chan, self.net, self.loc, self.time.isoformat("T"), self.phase, self.modified_id, self.assoc_id)
...
class PickModified(Base):
    ...
    def __repr__(self):
        return "Pick <%s.%s.%s.%s %s %s %s %s>" % (
        self.sta, self.chan, self.net, self.loc, self.time.isoformat("T"), self.phase, self.uncert, self.assoc_id)
...
class Candidate(Base):
    ...
    def __repr__(self):
        ...
        self.ot.isoformat("T"), self.sta, self.d_km, self.delta, self.p_modified_id, self.s_modified_id)
...
class Associated(Base):
    ...
    def __init__(self, ot, ot_uncert, latitude, longitude, depth, rms, nsta, t_create, t_update):
        self.ot = ot
    ...
    def __repr__(self):
        return "Associated Event <%s %s %.3f %.3f %d>" % (
        self.ot.isoformat("T"), self.ot, self.latitude, self.longitude, self.nsta)
