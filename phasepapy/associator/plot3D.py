...
class Plot():
    ...
    """
        ...
        for match in matches:
            lon, lat = self.tt_stations_db_3D.query(Station3D.longitude, Station3D.latitude).filter(
                Station3D.sta == match.sta).first()
        ...
        for mismatch in mismatches:
            lon, lat = self.tt_stations_db_3D.query(Station3D.longitude, Station3D.latitude).filter(
                Station3D.sta == mismatch.sta).first()
        ...
        for single_phase in single_phases:
            lon, lat = self.tt_stations_db_3D.query(Station3D.longitude, Station3D.latitude).filter(
                Station3D.sta == single_phase.sta).first()
            x, y = m(lon, lat)
    ...
    def section_plot(self, assoc_id, files, seconds_ahead=5, record_length=100, channel='Z'):
        ...
        for tr in ST_new:
            ...
            distance = int(gps2DistAzimuth(lat, lon, eq_lat, eq_lon)[
