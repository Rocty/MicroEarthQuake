...
def data_process(data):
    ...
    data_max = np.stack([np.max(data_input, axis=2)] * data.shape[1], axis=2) + 0.00001
    data_input = data_input / data_max
    return data_input
...
class CNN(object):
    def __init__(self):
        ...
        self.train_writer = tf.summary.FileWriter('eqdetector/summary/cnn')
    ...
    def setup_layer(self):
        ...
        layer['input'] = tf.placeholder(tf.float32,
                                        shape=[None, 1, self.config.detect_window_size * 100, 3],
                                        name='input')
    ...
    def train(self, passes, new_training=True):
        with tf.Session() as sess:
            ...
            if new_training:
                ...
                saver, global_step = Model.continue_previous_session(sess,
                                                                     model_file='cnn',
                                                                     ckpt_file='eqdetertor/saver/cnn/checkpoint')
            ...
            for step in range(1 + global_step, 1 + passes + global_step):
                ...
                if step % 100 == 0:
                    ...
                    saver.save(sess, 'eqdetertor/saver/cnn/cnn', global_step=step)
